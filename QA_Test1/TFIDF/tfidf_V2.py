...
class TF_IDF():
    ...
    def get_seg_docs(self):
        _seg_docs = []
        FOLDER_NAME = 'data'
        DOCUMENT = 'data_' + str(self.num) + '_512.json'
        ...
        for i in range(len(self.docs)):
            content_seg = [w for w in jieba.lcut(self.docs[i]['content']) if
                           len(w) > 1 and w not in self.stopword and w.isalpha()]
            _seg_docs.append(content_seg)
            self.id = self.docs[i]['id']
        return _seg_docs
...
def main():
    ...
    for x in range(1, args.nums):
        tf_idf = TF_IDF(num=x)
        topK = tf_idf.get_topK_idf(20, True)
        # 保存bag of word
        ...
        score1 = cosine_similarity(vec1, vec1)
        score2 = cosine_similarity(vec1, vec2)
